...
- **⚡ Cổng tự động nhập ảnh (P2)**: ảnh đủ nét (chấm chất lượng ngay trên trình duyệt) + AI tin cậy ≥ 0,92 + không cảnh báo + không trùng → **hệ thống TỰ tạo hồ sơ + tài khoản + ghi vết ledger**, không cần bấm xác nhận; thiếu 1 điều kiện → nâng cán bộ kèm lý do. Nhật kí toàn bộ lần AI đọc ảnh ở `/admin/ai-jobs`.
...
- **🧠 AI đọc ảnh OFFLINE (OCR-VN)**: PP-OCR dò chữ + VietOCR đọc tiếng Việt có dấu + PP-OCR đọc số; chạy trên CPU, ảnh không rời máy chủ. Đúng **95,8%** trường trên bộ 72 ảnh (Tesseract: 72,6%), ~2 giây/ảnh, tự trả RAM khi rảnh; tự quay về Tesseract nếu dịch vụ tắt. Cài: `sudo bash ai/ocr-vn/install.sh` (xem `ai/ocr-vn/README.md`, `docs/13-ai-doc-anh-ocr-vn.md`).
